// Mirrors SuspendInstructorDialog.tsx, learner violation taxonomy (the
// LEARNER-relevant subset of the shared users.validator.js VIOLATION_TYPES —
// see types/learners.ts LearnerViolationType).

import { useState } from 'react';
import { suspendLearner } from '../../services/learnersApi';
import { LearnerApiError } from '../../types/learners';
import type { LearnerViolationType } from '../../types/learners';
...
const VIOLATION_TYPES: { value: LearnerViolationType; label: string }[] = [
...
export default function SuspendLearnerDialog({ learnerId, fullName, onClose, onSuccess, showToast }: Props) {
  const [violationType, setViolationType] = useState<LearnerViolationType | ''>('');
  ...
  async function handleSubmit() {
    ...
    if (trimmed.length < 3) { setReasonErr('Reason must be at least 3 characters.'); hasError = true; }
    if (hasError) return;
    setReasonErr('');
    setViolationErr('');
    setSubmitting(true);
    try {
      await suspendLearner(learnerId, {
        reason: trimmed,
        violationType: violationType as LearnerViolationType,
        ...(notes.trim() ? { notes: notes.trim() } : {}),
      });
      invalidateFor(appQueryClient, 'learner.suspend', { id: learnerId });
      showToast('success', `${fullName} has been suspended.`);
      onSuccess();
    } catch (err) {
      if (err instanceof LearnerApiError) {
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      ...
      <div style={{ position: 'relative', background: '#fff', borderRadius: 12, width: '100%', maxWidth: 420, padding: 24, boxShadow: '0 20px 50px rgba(0,0,0,0.2)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
          ...
          <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: '#111827' }}>Suspend Learner</h3>
        </div>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <select
            ...
            value={violationType}
            onChange={e => { setViolationType(e.target.value as LearnerViolationType); setViolationErr(''); }}
