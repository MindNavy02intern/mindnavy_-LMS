// Learner Documents tab (side panel → More) — mirrors InstructorDocumentsTab.tsx
// exactly, different type taxonomy (IDENTITY/ENROLLMENT_AGREEMENT/
// ACADEMIC_RECORD/COMPLIANCE_FORM/CERTIFICATE). downloadUrl is a signed link
// that expires in 5 min — never cached, refetched fresh on every Download click.

import { useCallback, useEffect, useState } from 'react';
import { Download, Trash2, CheckCircle, XCircle, Upload } from 'lucide-react';
import {
  archiveLearnerDocument, confirmLearnerDocument, listLearnerDocuments,
  rejectLearnerDocument, signLearnerDocument, verifyLearnerDocument,
} from '../../services/learnersApi';
import { LearnerApiError } from '../../types/learners';
import type { LearnerDocument, LearnerDocumentType } from '../../types/learners';
...
const TYPE_OPTIONS: { value: LearnerDocumentType; label: string }[] = [
...
export default function LearnerDocumentsTab({ learnerId, showToast }: Props) {
  const [documents, setDocuments] = useState<LearnerDocument[] | null>(null);
  ...
  const [uploadType, setUploadType] = useState<LearnerDocumentType | ''>('');
  const [uploadErr, setUploadErr] = useState<string | null>(null);
  const [uploading, setUploading] = useState(false);

  const [rejectTarget, setRejectTarget] = useState<LearnerDocument | null>(null);
  ...
  const fetchList = useCallback(() => {
    setLoading(true);
    setListError(null);
    listLearnerDocuments(learnerId)
      .then(res => setDocuments(res.documents))
      .catch(err => setListError(err instanceof LearnerApiError ? err.message : 'Failed to load documents.'))
      .finally(() => setLoading(false));
  }, [learnerId]);

  useEffect(() => { fetchList(); }, [fetchList]);

  async function handleDownload(doc: LearnerDocument) {
    setBusyId(doc.id);
    try {
      const res = await listLearnerDocuments(learnerId);
      ...
      showToast('error', err instanceof LearnerApiError ? err.message : 'Download failed.');
    } finally {
      setBusyId(null);
    }
  }

  async function handleVerify(doc: LearnerDocument) {
    setBusyId(doc.id);
    try {
      await verifyLearnerDocument(learnerId, doc.id);
      invalidateFor(appQueryClient, 'learnerDoc.verify', { id: learnerId });
      showToast('success', `"${doc.fileName}" verified.`);
      fetchList();
    } catch (err) {
      showToast('error', err instanceof LearnerApiError ? err.message : 'Verify failed.');
    } finally {
      setBusyId(null);
    }
  }

  function openReject(doc: LearnerDocument) {
    setRejectTarget(doc);
    setRejectReason('');
    setRejectErr(null);
  }
  ...
  async function submitReject() {
    ...
    setRejectBusy(true);
    try {
      await rejectLearnerDocument(learnerId, rejectTarget.id, trimmed);
      invalidateFor(appQueryClient, 'learnerDoc.reject', { id: learnerId });
      showToast('success', `"${rejectTarget.fileName}" rejected.`);
      setRejectTarget(null);
      fetchList();
    } catch (err) {
      showToast('error', err instanceof LearnerApiError ? err.message : 'Rejection failed.');
    } finally {
      setRejectBusy(false);
    }
  }

  async function handleDelete(doc: LearnerDocument) {
    if (!window.confirm(`Remove "${doc.fileName}"? The file stays on record for compliance — this just hides it from the list.`)) return;
    setBusyId(doc.id);
    try {
      await archiveLearnerDocument(learnerId, doc.id);
      invalidateFor(appQueryClient, 'learnerDoc.archive', { id: learnerId });
  ...
  async function submitUpload() {
    ...
    setUploading(true);
    try {
      const sign = await signLearnerDocument(learnerId, {
      ...
      if (!putRes.ok) {
        ...
        return; // do not call confirm — the upload never landed
      }
      await confirmLearnerDocument(learnerId, { path: sign.path, fileName: uploadFile.name, type: uploadType });
      invalidateFor(appQueryClient, 'learnerDoc.upload', { id: learnerId });
      showToast('success', `"${uploadFile.name}" uploaded.`);
      setUploadOpen(false);
      fetchList();
    } catch (err) {
      showToast('error', err instanceof LearnerApiError ? err.message : 'Upload failed.');
    } finally {
      setUploading(false);
    }
  }
                ...
                {documents.map(doc => {
                  ...
                  return (
                    <tr key={doc.id} style={busy ? { opacity: 0.5 } : undefined}>
                      <td style={{ ...TD, maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={doc.fileName}>{doc.fileName}</td>
                      <td style={TD}>
                      ...
                      <td style={TD}>
                        ...
                        {doc.status === 'REJECTED' && doc.rejectionReason && (
                          <div style={{ fontSize: 10, color: '#94a3b8', marginTop: 2, maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis' }} title={doc.rejectionReason}>
                            {doc.rejectionReason}
                          </div>
                        )}
                      </td>
            ...
            <div style={{ padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div>
                ...
                <select
                  ...
                  value={uploadType}
                  onChange={e => { setUploadType(e.target.value as LearnerDocumentType); setUploadErr(null); }}
                  style={TA}
                >
