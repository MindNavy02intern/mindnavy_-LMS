...
export default function LmGuide({ onCreateCourse }: Props) {
  return (
    <div className="tw:rounded-xl tw:border tw:border-slate-200 tw:bg-white tw:p-5">
      ...
      <div className="tw:mt-3 tw:flex tw:flex-col tw:gap-3.5">
        ...
        {/* Remaining 4 — not yet wired; visibly disabled so they don't mislead */}
        {([
        ...
        ] as const).map(({ title, desc, Icon, iconBg, iconColor }) => (
          <button
            key={title}
            type="button"
            disabled
            title="Coming soon"
            className="tw:flex tw:cursor-not-allowed tw:items-start tw:gap-3 tw:text-left tw:opacity-40"
          >
