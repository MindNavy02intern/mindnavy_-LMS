...
const EditGroupModal: React.FC<Props> = ({ group, onClose, onSaved }) => {
  ...
  // Re-sync form when the group prop changes. Adjusted during render (React's
  // documented pattern for resetting state on prop change) instead of an
  // effect, so it isn't a synchronous setState-in-effect call.
  const [syncedGroupId, setSyncedGroupId] = useState(group.id);
  if (group.id !== syncedGroupId) {
    setSyncedGroupId(group.id);
    setForm({
      name:         group.name,
      description:  group.description ?? '',
      departmentId: group.departmentId ?? '',
    ...
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
      <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '28px', maxWidth: '500px', width: '90%', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
        <div style={{ marginBottom: '20px' }}>
          <h2 style={{ margin: 0, fontSize: '18px', fontWeight: 700, color: '#111827' }}>✏️ Edit Group</h2>
          <p style={{ margin: '4px 0 0', fontSize: '13px', color: '#6b7280' }}>Update the details for <strong>{group.name}</strong>.</p>
        </div>
