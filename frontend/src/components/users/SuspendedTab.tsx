    ...
    <tr style={{ borderBottom: '1px solid #f0f0f0' }}>
      ...
      <td style={{ padding: '10px 14px' }}>
        <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
          <div style={sk(70, 26)} /><div style={sk(56, 26)} />
        </div>
      </td>
    </tr>
  );
...
export default function SuspendedTab() {
  ...
  const [confirmType, setConfirmType] = useState<'reactivate' | 'archive' | null>(null);
  ...
  const load = useCallback(() => {
    setLoading(true);
    setError(null);
    getUsers({ status: 'suspended', page, limit: LIMIT })
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '0 0 8px 8px', overflow: 'hidden' }}>
      ...
      {actionError && (
        <div style={{
          margin: '10px 14px 0', padding: '8px 12px', fontSize: 12,
          background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 6, color: '#b91c1c',
          display: 'flex', justifyContent: 'space-between', alignItems: 'center',
        }}>
          {actionError}
          <button onClick={() => setActionError(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#b91c1c', fontSize: 14, lineHeight: 1 }}>×</button>
        </div>
      )}
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 700 }}>
        <thead>
          <tr>
            ...
            <th style={TH}>Suspended Since</th>
              ...
              ? (
                <tr>
                  <td colSpan={6} style={{ padding: '3rem', textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#374151' }}>No suspended users</div>
                    <div style={{ fontSize: 12, color: '#9ca3af', marginTop: 4 }}>All users are currently active or in another state.</div>
                  </td>
                </tr>
              )
              : users.map(user => {
                        ...
                        {isConfirming ? (
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 6 }}>
                            <span style={{ fontSize: 12, color: '#374151' }}>
                              {confirmType === 'reactivate' ? 'Reactivate?' : 'Archive?'}
                            </span>
                            <button
                              onClick={() => confirmType === 'reactivate' ? handleReactivate(user.id) : handleArchive(user.id)}
                              disabled={isBusy}
                              style={{ fontSize: 12, fontWeight: 600, padding: '4px 10px', borderRadius: 5, border: 'none', cursor: 'pointer', fontFamily: 'inherit', color: '#fff', background: confirmType === 'reactivate' ? '#16a34a' : '#dc2626' }}
                            >
                              Confirm
                            </button>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 6 }}>
                            <button
                              onClick={() => { setConfirmId(user.id); setConfirmType('reactivate'); }}
                              disabled={isBusy}
                              style={{ fontSize: 12, fontWeight: 600, padding: '5px 10px', borderRadius: 5, border: '1px solid #bbf7d0', background: '#f0fdf4', cursor: 'pointer', fontFamily: 'inherit', color: '#16a34a', display: 'flex', alignItems: 'center', gap: 4 }}
                            ...
                            <button
                              onClick={() => { setConfirmId(user.id); setConfirmType('archive'); }}
                              disabled={isBusy}
      ...
      {totalPages > 1 && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 14px', borderTop: '1px solid #e5e7eb', fontSize: 12, color: '#6b7280' }}>
          <span>{total.toLocaleString()} suspended user{total !== 1 ? 's' : ''}</span>
