...
export default function InvitationsTab() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    setError(null);
    getUsers({ status: 'invited', page, limit: LIMIT })
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '0 0 8px 8px', overflow: 'hidden' }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 700 }}>
        <thead>
          <tr>
            <th style={TH}>Invited User</th>
            <th style={TH}>Email</th>
            <th style={TH}>Role</th>
            <th style={TH}>Department</th>
            <th style={TH}>Invited Date</th>
              ...
              ? (
                <tr>
                  <td colSpan={6} style={{ padding: '3rem', textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#374151' }}>No pending invitations</div>
                    <div style={{ fontSize: 12, color: '#9ca3af', marginTop: 4 }}>Invited users will appear here until they accept.</div>
                  </td>
                </tr>
              )
              : users.map(user => {
                  ...
                  const justResent = resentId === user.id;
                  return (
                    <tr key={user.id}
                      ...
                      <td style={{ ...TD, textAlign: 'right' }}>
                        {isConfirming ? (
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 6 }}>
                            <span style={{ fontSize: 12, color: '#374151' }}>Cancel invitation?</span>
                            <button onClick={() => handleCancel(user.id)} disabled={isBusy}
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 6 }}>
                            <button
                              ...
                              style={{
                                ...
                                border: `1px solid ${justResent ? '#bbf7d0' : '#bfdbfe'}`,
                                background: justResent ? '#f0fdf4' : '#eff6ff',
                                cursor: 'pointer', fontFamily: 'inherit',
                                color: justResent ? '#16a34a' : '#2563eb',
                              }}
                              onMouseEnter={e => { if (!justResent) { e.currentTarget.style.background = '#dbeafe'; } }}
                              onMouseLeave={e => { if (!justResent) { e.currentTarget.style.background = '#eff6ff'; } }}
                            >
                              {justResent ? '✓ Sent' : '✉ Resend'}
                            </button>
                            <button
      ...
      {totalPages > 1 && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 14px', borderTop: '1px solid #e5e7eb', fontSize: 12, color: '#6b7280' }}>
          <span>{total.toLocaleString()} invitation{total !== 1 ? 's' : ''} pending</span>
