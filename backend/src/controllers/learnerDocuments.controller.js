const svc = require("../services/learnerDocuments.service");
...
const {
  ...
  validateReject,
  validateListQuery,
} = require("../validators/learnerDocuments.validator");
...
function handleDomainError(res, err) {
  switch (err.code) {
    ...
    case "DOCUMENT_ARCHIVED":
      return res.status(409).json({ success: false, message: "This document is archived. Upload a replacement instead." });
    case "BAD_PATH":
...
function serverError(res, err) {
  console.error("[LearnerDocumentsController]", err);
